...
% Vectorized function fitnessclustsse
%
% Input: 
%   centers - a (kp x N) matrix of candidate solutions, where each column
%             vector represents the positions of k cluster centers.  That
%             is, the dimensionality of the optimization problem is kp.
%   data    - an (p x n) dataset to be clustered (n data points of
%             dimensionality p)
% Output:
%   fval    - a (N x 1) vector of fitness values
%
% By: Mario Andres Munoz Acosta (mario.munoz@monash.edu)
%     30/07/2015
%
% Based on code by Marcus Gallagher

% Formatting the data
[p,n]   = size(data);
[kp,N]  = size(centers);
k       = kp./p;
centers = reshape(centers,p,k,N);
% data    = reshape(data,p,1,n);
% Pairwise distances of all data points
D = arrayfun(@(i) L2_distance(centers(:,:,i),data),...
             1:N,'UniformOutput',false);
D = reshape(cell2mat(D),k,n,N);
% Find which cluster center is closest to each datapoint
[~,idx] = min(D,[],1);
mask    = false(size(D));
mask(sub2ind(size(mask),idx(:),...
                        repmat(transpose(1:n),N,1),...
                        reshape(repmat(1:N,n,1),n*N,1))) = true;
fval = squeeze(sum(sum((D.*mask).^2,2),1));
